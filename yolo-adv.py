...
class EnhancedTrafficAnalysisApp:
    ...
    def setup_ui(self):
        ...
        # Right panel for visualizations with notebook tabs
        self.viz_notebook = ttk.Notebook(self.content_frame)
        self.viz_notebook.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def resize_frame(self, frame):
        ...
        try:
            # Get current canvas dimensions - crucial for constraining the video
            canvas_width = self.video_canvas.winfo_width()
            canvas_height = self.video_canvas.winfo_height()

            # If canvas hasn't been drawn yet, use default dimensions
            if canvas_width <= 1:
                # Use a percentage of the content frame width to leave room for viz panel
                content_width = self.content_frame.winfo_width()
                if content_width > 1:
                    canvas_width = int(content_width * 0.6)  # Use 60% of width
                else:
                    canvas_width = 640
    ...
    def setup_plots(self):
        # Set up plots for each tab

        # 1. Counts tab
        self.counts_fig = Figure(figsize=(5, 4), dpi=100)
        ...
        # 2. Time series tab
        self.time_fig = Figure(figsize=(5, 4), dpi=100)
        ...
        # 3. Heatmap tab
        self.heatmap_fig = Figure(figsize=(5, 4), dpi=100)
        ...
        # 4. Direction tab
        self.direction_fig = Figure(figsize=(5, 4), dpi=100)
        ...
        # 5. Speed tab
        self.speed_fig = Figure(figsize=(5, 4), dpi=100)
    ...
    def draw_visualizations(self, frame, detections):
        ...
        # Return the frame with visualizations
        return frame
    ...
    def setup_spatial_tab(self, parent):
        ...
        # Bottom right frame for ROIs
        roi_frame = ttk.LabelFrame(frame, text="Region of Interest Analysis")
        roi_frame.grid(row=1, column=1, padx=5, pady=5, sticky="nsew")

        # Create a table of ROI statistics
        roi_columns = ("Region", "Count", "Density")
        roi_tree = ttk.Treeview(roi_frame, columns=roi_columns, show="headings")

        for col in roi_columns:
            roi_tree.heading(col, text=col)
            roi_tree.column(col, width=100, anchor="center")
        ...
        for roi in self.regions_of_interest:
            ...
            points = roi["points"]
            if len(points) > 2:
                # Simple approximation for polygon area
                area = 0
                ...
                # Density = count per 1000 square pixels
                density = (count / area) * 1000 if area > 0 else 0

                roi_tree.insert(
                    "", "end", values=(name, count, f"{density:.2f}/1000px²")
                )

        # Add scrollbar
        roi_scrollbar = ttk.Scrollbar(
            roi_frame, orient="vertical", command=roi_tree.yview
        )
        roi_tree.configure(yscrollcommand=roi_scrollbar.set)
        roi_scrollbar.pack(side="right", fill="y")
        roi_tree.pack(fill="both", expand=True)
        ...
        def update_spatial():
            ...
            # Clear existing rows
            for item in roi_tree.get_children():
                roi_tree.delete(item)
    ...
    def show_instructions(self):
        ...
        • Add regions of interest (ROIs) to analyze traffic in specific areas
        • Click and drag on the video to position these zones
    ...
    def set_frame_skip(self):
        ...
        if value is not None:
            ...
            self.frame_skip_var.set(value)
            self.status_var.set(f"Frame skip set to {value}")

    def set_downscale(self):
        """Open dialog to set downscale factor"""
        value = simpledialog.askfloat(
            "Downscale Factor",
            "Enter downscale factor (0.2-1.0):\n\nLower values improve performance but reduce detection accuracy.",
            minvalue=0.2,
            maxvalue=1.0,
        ...
        if value is not None:
            ...
            self.status_var.set(f"Downscale factor set to {value:.1f}")
